function [fout, faux] = network_subtemp_slab_steady(v_in,parameters)
...
%initialize output
fout = zeros(length(v_in)+Q_nodes,1);
%%  PRELIMINARIES 

%DISCRETE DEP. VARIABLES
[discvar, ~] = discretisation_v3([v_in; T_bed], parameters);
...
fout(2*psi_nodes+2*T_nodes+1:2*psi_nodes+2*T_nodes+Q_nodes) = net_Qbedflux./Q_Delta_x_cell-source_Q;%
%% HEAT EQUATION ICE
...
fout(2*psi_nodes+1:2*psi_nodes+T_nodes) = net_verflux./T_Delta_z_cell + net_horflux./T_Delta_x_cell;
%% HEAT EQUATION BED
...
%enforce conservation law
fout(2*psi_nodes+T_nodes+1:2*psi_nodes+2*T_nodes) =  net_bedverflux./Tb_Delta_z_cell ;%+net_bedhorflux./Tb_Delta_x_cell; 
%% BASAL ENERGY BUDGET
