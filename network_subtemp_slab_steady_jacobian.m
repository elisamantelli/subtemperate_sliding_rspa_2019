function [fout, faux] = network_subtemp_slab_steady_jacobian(v_in,parameters)
...
T_bed = parameters.T_bed; %v_in(2*psi_nodes+2*T_nodes+Q_nodes+1:end);

%initialize output
fout = sparse(length(v_in)+Q_nodes,length(v_in));
faux = fout;
%%  PRELIMINARIES 

%DISCRETE DEP. VARIABLES
[discvar, Ddiscvar] = discretisation_v3([v_in;T_bed], parameters);
...
fout(1:psi_nodes,:) = [Dfoutpsi_dpsi Dfoutpsi_domega sparse(psi_nodes,2*T_nodes+Q_nodes)];
...
fout(psi_nodes+1:2*psi_nodes,:) = [ Dfoutomega_dpsi Dfoutomega_domega sparse(psi_nodes,2*T_nodes+Q_nodes)];
...
DfoutQ_dQ =   spdiags(1./Q_Delta_x_cell,0,Q_nodes,Q_nodes)*dQQ_dQ - dsourceterm_dQ;%
DfoutQ_dpsi =  spdiags(1./Q_Delta_x_cell,0,Q_nodes,Q_nodes)*dQQ_dpsi - dsourceterm_dpsi;
fout(2*psi_nodes+2*T_nodes+1:2*psi_nodes+2*T_nodes+Q_nodes, :) = [DfoutQ_dpsi sparse(Q_nodes, psi_nodes+2*T_nodes) DfoutQ_dQ];

DfoutQ_dQ_steady =   spdiags(1./Q_Delta_x_cell,0,Q_nodes,Q_nodes)*dQQ_dQ - dsourceterm_dQ;
faux(2*psi_nodes+2*T_nodes+1:2*psi_nodes+2*T_nodes+Q_nodes, :) = [DfoutQ_dpsi sparse(Q_nodes, psi_nodes+2*T_nodes) DfoutQ_dQ_steady ];
%% HEAT EQUATION 
...
%conservation law
DfoutT_dT =  spdiags(1./T_Delta_z_cell,0,T_nodes,T_nodes)*dQV_dT+ spdiags(1./T_Delta_x_cell,0,T_nodes,T_nodes)*dQH_dT;
...
fout(2*psi_nodes+1:2*psi_nodes+T_nodes,:) = [DfoutT_dpsi sparse(T_nodes,psi_nodes) DfoutT_dT sparse(T_nodes,T_nodes) DfoutT_dQ ];
faux(2*psi_nodes+1:2*psi_nodes+T_nodes,:) = [DfoutT_dpsi sparse(T_nodes,psi_nodes) DfoutT_dT_steady sparse(T_nodes,T_nodes) DfoutT_dQ ];
%% BED HEAT EQUATION
...
DfoutTb_dT = spdiags(1./Tb_Delta_z_cell,0,T_nodes,T_nodes)*dQVbed_dTb;
...
fout(2*psi_nodes+T_nodes+1:2*psi_nodes+2*T_nodes,:) = [sparse(T_nodes,2*psi_nodes+T_nodes) DfoutTb_dT sparse(T_nodes,Q_nodes) ];
faux(2*psi_nodes+T_nodes+1:2*psi_nodes+2*T_nodes,:) = [sparse(T_nodes,2*psi_nodes+T_nodes) DfoutTb_dT_steady sparse(T_nodes,Q_nodes) ];
%% BASAL ENERGY BUDGET
...
fout(2*psi_nodes+2*T_nodes+Q_nodes+1:2*psi_nodes+2*T_nodes+2*Q_nodes,:) =[ Dfoutv_dpsi sparse(Q_nodes,psi_nodes) Dfoutv_dT Dfoutv_dTb sparse(Q_nodes, Q_nodes) ];
